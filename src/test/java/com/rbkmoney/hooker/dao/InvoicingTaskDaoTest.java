...
public class InvoicingTaskDaoTest extends AbstractIntegrationTest {
    ...
    public void createDeleteGet() {
        ...
        Map<Long, List<Task>> scheduled = taskDao.getScheduled(50);
        assertEquals(1, scheduled.size());
        taskDao.remove(scheduled.keySet().iterator().next(), messageId);
        assertEquals(0, taskDao.getScheduled(50).size());
    }
    ...
    public void testSelectForUpdate() {
        ...
        new Thread(() -> transactionTemplate.execute(tr -> {
            scheduledOne.addAll(taskDao.getScheduled(50).values().stream().flatMap(List::stream).map(Task::getMessageId).collect(Collectors.toSet()));
        ...
        new Thread(() -> transactionTemplate.execute(tr -> {
            scheduledTwo.addAll(taskDao.getScheduled(50).values().stream().flatMap(List::stream).map(Task::getMessageId).collect(Collectors.toSet()));
