...
public class ComplexDataflowTest extends AbstractIntegrationTest {
    ...
    public void testMessageSend() throws InterruptedException {
        ...
        sourceMessages.addAll(Arrays.asList(
                buildMessage(InvoicingMessageEnum.INVOICE.value(),"1", "partyId1", EventType.INVOICE_STATUS_CHANGED, InvoiceStatusEnum.paid, PaymentStatusEnum.captured,0L, 0),
                buildMessage(InvoicingMessageEnum.PAYMENT.value(), "1", "partyId1", EventType.INVOICE_PAYMENT_STATUS_CHANGED, InvoiceStatusEnum.paid, PaymentStatusEnum.captured,0L, 1),
                buildMessage(InvoicingMessageEnum.PAYMENT.value(), "2", "partyId1", EventType.INVOICE_PAYMENT_STATUS_CHANGED, InvoiceStatusEnum.paid, PaymentStatusEnum.processed,0L, 0),
                buildMessage(InvoicingMessageEnum.PAYMENT.value(), "2", "partyId1", EventType.INVOICE_PAYMENT_STATUS_CHANGED, InvoiceStatusEnum.paid, PaymentStatusEnum.failed, 0L, 1),
                buildMessage(InvoicingMessageEnum.PAYMENT.value(), "3", "partyId1", EventType.INVOICE_PAYMENT_STATUS_CHANGED, InvoiceStatusEnum.paid, PaymentStatusEnum.failed, 0L, 1)));
        ...
        assertEquals(sourceMessages.get(0).getInvoiceStatus().toString(), hooks.get(0).getInvoice().getStatus().toString());
        assertEquals(sourceMessages.get(1).getPaymentStatus().toString(), hooks.get(1).getPayment().getStatus().toString());
        assertEquals(sourceMessages.get(3).getPaymentStatus().toString(), hooks.get(2).getPayment().getStatus().toString());
        ...
        assertEquals(sourceMessages.get(sourceMessages.size() - 1).getInvoiceId(), invoicingMessageFailed.getInvoiceId());
